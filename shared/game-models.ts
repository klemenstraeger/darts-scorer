...
export function turnTotalPoints(turn: Turn): number {
  if (turn.busted)
    return 0
  if (turn.visitScore !== undefined)
    return turn.visitScore
...
export function getCheckoutDart(player: Player): ThrowResult | null {
  if (player.score !== 0 || player.turns.length === 0)
    return null
  const lastTurn = player.turns[player.turns.length - 1]!
  if (lastTurn.busted || lastTurn.throws.length === 0)
    return null
